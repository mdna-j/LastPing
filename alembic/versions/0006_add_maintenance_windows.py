...
def upgrade() -> None:
    ...
    if dialect == 'sqlite':
        ...
        if "project" in inspector.get_table_names():
            ...
            if "maintenance_starts_at" not in project_cols:
                conn.execute(text('ALTER TABLE project ADD COLUMN maintenance_starts_at TIMESTAMP'))
            if "maintenance_ends_at" not in project_cols:
                conn.execute(text('ALTER TABLE project ADD COLUMN maintenance_ends_at TIMESTAMP'))
        if "check" in inspector.get_table_names():
            check_cols = {col["name"] for col in inspector.get_columns("check")}
            if "maintenance_starts_at" not in check_cols:
                conn.execute(text('ALTER TABLE "check" ADD COLUMN maintenance_starts_at TIMESTAMP'))
            if "maintenance_ends_at" not in check_cols:
                conn.execute(text('ALTER TABLE "check" ADD COLUMN maintenance_ends_at TIMESTAMP'))
