...
document.addEventListener('DOMContentLoaded', ()=>{ loadPrefs(); const s = document.getElementById('savePrefsBtn'); if(s) s.addEventListener('click', savePrefs); const e = document.getElementById('exportCsvBtn'); if(e) e.addEventListener('click', exportCsv); const api = document.getElementById('apiKey'); if(api) api.addEventListener('change', loadChecks); const pid = document.getElementById('projectId'); if(pid) pid.addEventListener('change', loadChecks); const presets = document.createElement('div'); presets.style.marginTop='8px'; presets.className='row'; presets.innerHTML = '<button class="btn btn-secondary" id="p1h">Last 1h</button> <button class="btn btn-secondary" id="p6h">Last 6h</button> <button class="btn btn-secondary" id="p24h">Last 24h</button> <button class="btn btn-secondary" id="p7d">Last 7d</button>'; const listEl = document.getElementById('list'); const parent = listEl ? listEl.parentNode : null; if(parent){ parent.insertBefore(presets, listEl); } else { document.body.appendChild(presets); }
